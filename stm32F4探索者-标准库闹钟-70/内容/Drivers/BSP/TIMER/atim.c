...
//    sys_gpio_af_set(ATIM_TIMX_NPWM_CHY_GPIO_PORT, ATIM_TIMX_NPWM_CHY_GPIO_PIN, ATIM_TIMX_NPWM_CHY_GPIO_AF); /* IO口复用功能选择 必须设置对!! */

//    ATIM_TIMX_NPWM->ARR = arr;       /* 设定计数器自动重装值 */
//    ATIM_TIMX_NPWM->PSC = psc;       /* 设置预分频器  */
//    ATIM_TIMX_NPWM->BDTR |= 1 << 15; /* 使能MOE位(仅TIM15/16/17 有此寄存器,必须设置MOE才能输出PWM), 其他高级定时器,
...
void atim_timx_cplm_pwm_init(uint16_t arr, uint16_t psc)
{
    ...
    sys_gpio_af_set(ATIM_TIMX_CPLM_CHY_GPIO_PORT,  ATIM_TIMX_CPLM_CHY_GPIO_PIN,  ATIM_TIMX_CPLM_CHY_GPIO_AF);   /* IO口复用功能选择 必须设置对!! */
    sys_gpio_af_set(ATIM_TIMX_CPLM_CHYN_GPIO_PORT, ATIM_TIMX_CPLM_CHYN_GPIO_PIN, ATIM_TIMX_CPLM_CHYN_GPIO_AF);  /* IO口复用功能选择 必须设置对!! */
 ...
 * @note
 *              因为TIM1/TIM8等有多个中断服务函数,而TIM2~5/TIM9~TIM14等普通定时器只有1个中断服务
 ...
 * @note
 *              通用定时器的时钟来自APB2,当PPRE1≥2分频的时候
 *              通用定时器的时钟为APB2时钟的2倍, 而APB2为84M, 所以定时器时钟 = 168Mhz
 *              定时器溢出时间计算方法: Tout = ((arr + 1) * (psc + 1)) / Ft us.
 *              Ft=定时器工作频率,单位:Mhz
 *
 *              本函数初始化的时候: 使用psc=0, arr固定为65535. 得到采样时钟频率为168Mhz,精度5.95ns
 *
 * @param       无
 * @retval      无
 */
void atim_timx_pwmin_chy_init(void)
{
    ...
    sys_gpio_af_set(ATIM_TIMX_PWMIN_CHY_GPIO_PORT, ATIM_TIMX_PWMIN_CHY_GPIO_PIN, ATIM_TIMX_PWMIN_CHY_GPIO_AF);  /* IO口复用功能选择 必须设置对!! */
