 ...
 * 默认是针对TIM2~TIM5, TIM9~TIM14. 
 * 注意: 通过修改这10个宏定义,可以支持TIM1~TIM14任意一个定时器,任意一个IO口做输入捕获
...
#define GTIM_TIMX_CAP_CHY_GPIO_CLK_ENABLE() do{ RCC->AHB1ENR |= 1 << 0; }while(0)   /* PA口时钟使能 */
...
* 默认是针对TIM2~TIM5, TIM9~TIM14.  只有CH1和CH2通道可以用做输入计数, CH3/CH4不支持!
* 注意: 通过修改这9个宏定义,可以支持TIM1~TIM14任意一个定时器,CH1/CH2对应IO口做输入计数
*       特别要注意:默认用的PA0,设置的是下拉输入!如果改其他IO,对应的上下拉方式/AF功能等也得改!
*/
#define GTIM_TIMX_CNT_CHY_GPIO_PORT         GPIOA
#define GTIM_TIMX_CNT_CHY_GPIO_PIN          SYS_GPIO_PIN0
#define GTIM_TIMX_CNT_CHY_GPIO_AF           1                           /* AF功能选择 */
#define GTIM_TIMX_CNT_CHY_GPIO_CLK_ENABLE() do{ RCC->AHB1ENR |= 1 << 0; }while(0)   /* PA口时钟使能 */
